<?
/*
 Fichier : 2B_gazpar.php 
 Version : 0.0.0
 
 Source original :  https://github.com/beufanet/gazpar
...
class sdk_cookies
{
    ...
    public function sdk_dump()
    {
        var_dump($cookies);
    }

    public function sdk_parse($hdr)
    {
     	preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $hdr, $matches);
...
function sdk_gazpar_login($username, $password)
{
	...
	$headers = array (
                ...
                'Cookie: KPISavedRef='.urlencode('https://monespace.grdf.fr/monespace/connexion'));
	...
	$url = 'https://monespace.grdf.fr/monespace/connexion';
	$req = httpQuery($url, 'GET', NULL, NULL, $headers, false, false, $info);
	
	$cookies = array();
	preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $info['header'], $matches);
    foreach($matches[1] as $item) 
    {
        parse_str($item, $cookie);
        $cookies = array_merge($cookies, $cookie);
	...
	$url = $GLOBALS["LOGIN_BASE_URI"] . $GLOBALS["API_ENDPOINT_LOGIN"];
	$req = httpQuery($url, 'POST', $payload, NULL, $headers, false, false, $info);
	var_dump($info);
	...
	$GLOBALS["JAVAVXS"] = xpath($req, "//update[@id = 'javax.faces.ViewState']");
    ...
    $payload = array (
               ...
               'javax.faces.behavior.event: click',
               'javax.faces.partial.event: click',
               'javax.faces.ViewState: '.$GLOBALS["JAVAVXS"],
               '_EspacePerso_WAR_EPportlet_:seConnecterForm: _EspacePerso_WAR_EPportlet_:seConnecterForm',
               'javax.faces.encodedURL: https://monespace.grdf.fr/web/guest/monespace?p_p_id=EspacePerso_WAR_EPportlet&amp;p_p_lifecycle=2&amp;p_p_state=normal&amp;p_p_mode=view&amp;p_p_cacheability=cacheLevelPage&amp;p_p_col_id=column-2&amp;p_p_col_count=1&amp;_EspacePerso_WAR_EPportlet__jsfBridgeAjax=true&amp;_EspacePerso_WAR_EPportlet__facesViewIdResource=%2Fviews%2FespacePerso%2FseconnecterEspaceViewMode.xhtml',
               '_EspacePerso_WAR_EPportlet_:seConnecterForm:email: '.$username,
               '_EspacePerso_WAR_EPportlet_:seConnecterForm:passwordSecretSeConnecter: '.$password
...
sdk_gazpar_login('user', 'password');
die();
?>
